class PagesController < ApplicationController
  def inventory
    ...
    @supplyQuantity = Supply.joins(:inventories)
           .select('inventories.supply_id,supplies.name,inventories.quantity, supplies.suppliesid')
    ...
    @oInfo = Supplier.joins("INNER JOIN costs ON costs.supplier_id = suppliers.id INNER JOIN supplies ON supplies.suppliesid = costs.supply_id").select("supplies.name, suppliers.name, costs.*")
